...
export function ProviderList({
  ...
  // Fusion 模式下收集所有活跃的 providerId
  const fusionProviderIds = useMemo(() => {
    const fusion = settings?.fusionModelMapping;
    if (!fusion?.enabled || appId !== "claude") return new Set<string>();
    const ids = new Set<string>();
    if (fusion.haiku?.providerId) ids.add(fusion.haiku.providerId);
    if (fusion.sonnet?.providerId) ids.add(fusion.sonnet.providerId);
    if (fusion.opus?.providerId) ids.add(fusion.opus.providerId);
    if (fusion.default?.providerId) ids.add(fusion.default.providerId);
    return ids;
  }, [settings?.fusionModelMapping, appId]);
          ...
          {filteredProviders.map((provider) => {
            ...
            return (
              <SortableProviderCard
                ...
                isFusionActive={fusionProviderIds.has(provider.id)}
                onSetAsDefault={
