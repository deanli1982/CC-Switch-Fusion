...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "cc-switch-fusion-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "cc-switch-fusion-sync",
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "cc-switch-fusion-sync";
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "cc-switch-fusion-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "cc-switch-fusion-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Input
              ...
              placeholder="cc-switch-fusion-sync"
              className="text-xs flex-1"
              disabled={isLoading}
            />
          </div>
