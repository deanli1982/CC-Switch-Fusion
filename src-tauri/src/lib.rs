...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.cc-switch-fusion/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/cc-switch-fusion.log）
            {
                ...
                let log_file_path = log_dir.join("cc-switch-fusion.log");
            ...
            let db_path = app_config_dir.join("cc-switch-fusion.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 CC Switch Fusion 的工作方式。
            ...
            let mut tray_builder = TrayIconBuilder::with_id(tray::TRAY_ID)
                .tooltip("CC Switch Fusion") // 鼠标悬停提示
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 cc-switch-fusion.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including cc-switch-fusion.db)\n\
