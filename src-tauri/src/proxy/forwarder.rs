...
pub struct RequestForwarder {
    ...
    /// Fusion-mapped model name (cross-provider routing), overrides per-provider mapping
    fusion_mapped_model: Option<String>,
    /// 非流式请求超时（秒）
...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        // Fusion 路由优先级最高：如果 fusion_mapped_model 已设置，直接使用它，跳过 per-provider 映射。
        let mapped_body = if let Some(ref fusion_model) = self.fusion_mapped_model {
            let mut b = body.clone();
            b["model"] = serde_json::json!(fusion_model);
            b
