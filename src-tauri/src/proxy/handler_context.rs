...
pub struct RequestContext {
    ...
    /// Fusion-mapped model name (cross-provider routing), None = no fusion
    pub fusion_mapped_model: Option<String>,
...
impl RequestContext {
    ...
    pub async fn new(
        ...
        // Fusion Model Routing: check if cross-provider mapping applies
        let settings = crate::settings::get_settings();
        let (fusion_target, fusion_mapped_model) =
            if let Some(ref fusion) = settings.fusion_model_mapping {
                match crate::proxy::fusion_router::try_fusion_route(
                    &state.db,
                    app_type_str,
                    fusion,
                    &request_model,
                ...
                .await
                {
                    Ok(Some((provider, mapped))) => {
                        log::info!(
                            "[{}] Fusion: {} ({}) → provider={}, model={}",
                            tag,
                            request_model,
                            crate::proxy::fusion_router::classify_model_type(&request_model),
                            provider.name,
                            mapped
                        );
                        (Some(provider), Some(mapped))
                    }
                    _ => (None, None),
                }
            } else {
        ...
        // 如果 Fusion 路由已解析目标 provider，则直接使用它
        let (provider, providers, fusion_mapped_model) =
            if let Some(fusion_prov) = fusion_target {
                let p = fusion_prov.clone();
                (fusion_prov, vec![p], fusion_mapped_model)
            } else {
                let providers = state
                    .provider_router
    ...
    pub fn create_forwarder(&self, state: &ProxyState) -> RequestForwarder {
        ...
        RequestForwarder::new(
            ...
            self.fusion_mapped_model.clone(),
        )
    }
