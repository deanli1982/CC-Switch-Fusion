...
///   且不一定等于用户目录，可能导致 `.cc-switch/cc-switch-fusion.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    #[cfg(windows)]
    {
        let default_db = default_dir.join("cc-switch-fusion.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".cc-switch");
                    if legacy_dir.join("cc-switch-fusion.db").exists() {
                        log::info!(
