...
fn default_remote_root() -> String {
    "cc-switch-fusion-sync".to_string()
}
...
/// Claude model type classifier for fusion routing
...
/// Single fusion mapping entry: maps a model type to a (provider_id, model_name) pair
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct FusionMappingEntry {
    pub provider_id: String,
    pub model_name: String,
}

/// Cross-provider fusion model mapping configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct FusionModelMapping {
    #[serde(default)]
    pub enabled: bool,
    /// 检测到图片时自动按 Haiku 类型路由
    #[serde(default)]
    pub auto_image_to_haiku: bool,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub haiku: Option<FusionMappingEntry>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub sonnet: Option<FusionMappingEntry>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub opus: Option<FusionMappingEntry>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub default: Option<FusionMappingEntry>,
}

/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.cc-switch-fusion/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    /// Cross-provider fusion model mapping (model_type -> provider_id + model_name)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub fusion_model_mapping: Option<FusionModelMapping>,
}
...
mod tests {
    ...
    #[test]
    fn fusion_mapping_deserializes_from_none() {
        let settings: AppSettings = serde_json::from_str("{}").unwrap();
        assert!(settings.fusion_model_mapping.is_none());
    }

    #[test]
    fn fusion_mapping_serializes_when_none() {
        let settings = AppSettings::default();
        let json = serde_json::to_value(&settings).unwrap();
        assert!(json.get("fusionModelMapping").is_none());
    }

    #[test]
    fn fusion_mapping_roundtrip() {
        let mapping = FusionModelMapping {
            enabled: true,
            auto_image_to_haiku: false,
            haiku: Some(FusionMappingEntry {
                provider_id: "deepseek-1".to_string(),
                model_name: "deepseek-v4-pro[1m]".to_string(),
            }),
            sonnet: Some(FusionMappingEntry {
                provider_id: "qwen-1".to_string(),
                model_name: "qwen3.6-plus".to_string(),
            }),
            opus: None,
            default: None,
        };
        let settings = AppSettings {
            fusion_model_mapping: Some(mapping.clone()),
            ..AppSettings::default()
        };
        let json = serde_json::to_value(&settings).unwrap();
        assert_eq!(json["fusionModelMapping"]["enabled"], true);
        assert_eq!(json["fusionModelMapping"]["haiku"]["providerId"], "deepseek-1");
        assert_eq!(json["fusionModelMapping"]["sonnet"]["modelName"], "qwen3.6-plus");

        let parsed: AppSettings = serde_json::from_value(json).unwrap();
        let parsed_mapping = parsed.fusion_model_mapping.unwrap();
        assert_eq!(parsed_mapping.enabled, true);
        assert_eq!(parsed_mapping.haiku.unwrap().provider_id, "deepseek-1");
    }
}
